import { homeSections, sectionData } from '@/services/home.service';
...
export default async function Home() {
  ...
  const sectionsWithData = await Promise.all(
    sections.map(async (section: any) => {
      const res = await sectionData(section.id);
      return { ...section, data: res };
    })
  );
  ...
  return (
    <>
      ...
      <div className='flex flex-col flex-1 items-center justify-center gap-7.5 xl:gap-12.5 overflow-x-hidden'>
        <SectionsRenderer sections={sectionsWithData} />
      </div>
    </>
  );
}
